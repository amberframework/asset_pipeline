...
role: "article"
abstract: "An App Clip is a lightweight version of your app or game that provides an on-the-go or demo experience that’s instantly available."
...
![A sketch of an app icon surrounded by a dashed line, suggesting an App Clip. The image is overlaid with rectangular and circular grid lines and is tinted blue to subtly reflect the blue in the original six-color Apple logo.](../images/technologies-app-clips-intro.png)
...
![A screenshot of an iPhone Lock Screen. The bottom half of the screen shows the App Clip card for a donut shop’s App Clip as it appears when the person invokes the App Clip.](../images/app-clips-hero-1.png)

![A screenshot of a donut shop’s App Clip on iPhone as it appears when a person confirms the App Clip’s launch on the App Clip card. The App Clip displays a list with various donuts the person can order.](../images/app-clips-hero-2.png)
...
![A horizontal row of two App Clip cards. The left App Clip card is for a game and uses Play as the verb for the action button. The right App Clip card is for an app and uses Open as the verb for the action button.](../images/app-clips-card.png)
...
![An App Clip Code that uses a badge design with the App Clip logo.](../images/with-appclip-logo.png)

![An App Clip Code that uses a design without the App Clip logo.](../images/without-appclip-logo.png)
...
![A scan-only App Clip Code with callouts for the center icon, visual code, and the App Clip logo.](../images/scan-only.png)
...
![An illustration that shows a circle that represents a cylindrical surface. Lines divide the circle into six segments of equal size. One segment represents an App Clip Code and shows how the code doesn’t cover more than one-sixth, or 60 degrees, of the surface’s circumference.](../images/app-clips-slice.png)
...
![A correctly placed App Clip Code in the upright position.](../images/upright-display-right.png)

![A checkmark in a circle to indicate correct usage.](../images/checkmark.png)

![An incorrectly placed App Clip Code that's rotated 90 degrees to the left.](../images/upright-display-wrong-1.png)

![An X in a circle to indicate an invalid App Clip Code.](../images/crossout.png)

![An incorrectly placed App Clip Code that's rotated 135 degrees to the right.](../images/upright-display-wrong-2.png)
...
![An App Clip Code that uses the badge design and has a minimum diameter of 3/4 inch (1.9 cm).](../images/sizing-minimum-rectangle.png)

![An App Clip Code that uses the design without the App Clip logo and has a minimum diameter of 3/4 inch (1.9 cm).](../images/sizing-minimum-circular.png)
...
![An illustration of an App Clip Code next to a QR code. Red guides denote that both are the same size.](../images/app-clip-with-qr-code.png)
...
![An illustration that shows an App Clip Code with the badge design to the left of an App Clip Code without the App Clip logo. A red guide surrounds each App Clip Code, illustrating the clear space requirements.](../images/app-clip-spacing.png)
...
![Four App Clip badges, each using different colors. The two on the left use the badge design, and the two on the right use the design without the App Clip logo.](../images/app-clips-customizing.png)
...
![An illustration of an invalid App Clip Code with a changed aspect ratio.](../images/customizing-wrong-1.png)

![An X in a circle to indicate an invalid App Clip Code.](../images/crossout.png)

![An illustration of an invalid App Clip Code with a color gradient instead of a solid background color.](../images/customizing-wrong-2.png)

![An X in a circle to indicate an invalid App Clip Code.](../images/crossout.png)

![An illustration of an invalid App Clip Code with a drop shadow effect.](../images/customizing-wrong-3.png)
...
![An illustration of an App Clip Code that uses the badge design and has callouts for the background, foreground, and generated colors.](../images/app-clip-colors.png)
