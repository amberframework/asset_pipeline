module Components
  module CSS
    ...
    class ClassBuilder
      ...
      # WCAG accessibility variant methods

      def focus_visible(classes : String) : self
        @classes << classes.split(/\s+/).map { |c| "focus-visible:#{c}" }.join(" ")
        self
      end

      def focus_within(classes : String) : self
        @classes << classes.split(/\s+/).map { |c| "focus-within:#{c}" }.join(" ")
        self
      end

      def motion_safe(classes : String) : self
        @classes << classes.split(/\s+/).map { |c| "motion-safe:#{c}" }.join(" ")
        self
      end

      def motion_reduce(classes : String) : self
        @classes << classes.split(/\s+/).map { |c| "motion-reduce:#{c}" }.join(" ")
        self
      end

      def aria_expanded(classes : String) : self
        @classes << classes.split(/\s+/).map { |c| "aria-expanded:#{c}" }.join(" ")
        self
      end

      def aria_selected(classes : String) : self
        @classes << classes.split(/\s+/).map { |c| "aria-selected:#{c}" }.join(" ")
        self
      end

      def aria_checked(classes : String) : self
        @classes << classes.split(/\s+/).map { |c| "aria-checked:#{c}" }.join(" ")
        self
      end

      def aria_disabled(classes : String) : self
        @classes << classes.split(/\s+/).map { |c| "aria-disabled:#{c}" }.join(" ")
        self
      end

      def invalid(classes : String) : self
        @classes << classes.split(/\s+/).map { |c| "invalid:#{c}" }.join(" ")
        self
      end

      def valid(classes : String) : self
        @classes << classes.split(/\s+/).map { |c| "valid:#{c}" }.join(" ")
        self
      end
